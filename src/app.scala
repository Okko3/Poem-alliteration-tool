...
object Main extends App:
  ...
  userInputArray.foreach(line =>
    if line == "" then
      ...
      val cleanedline = line.replaceAll("^\\s+", " ").replace("”", "").replace("–", "").replace("-", "").replaceAll("\\s{2,}", " ").toLowerCase
    ...
    printf("%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\t%-1s\n", "Line", "Cat", "3CAT", "S-count", "W-count", "V1", "V1C", "V1%", "V2", "V2C", "V2%", "V3", "V3C", "V3%", "V4", "V4C", "V4%", "V1", "V1C", "V1%", "V2", "V2C", "V2%", "V3", "V3C", "V3%", "V4", "V4C", "V4%")
    ...
    for (i <- lines.indices) do
      ...
      val vokaalit3 = fourcommon2(i).padTo(12, "")
      printf("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n",
           ...
           vokaalit2(5), vokaalit2(6), vokaalit2(7), vokaalit2(8), vokaalit2(9), vokaalit2(10), vokaalit2(11))
...
object helper:
  ...
  def commonVowelPairs(line: String) =
    ...
    def isVowelPair(pair: String): Boolean =
      val vowels = "aeiouyäö"
      pair.length == 2 && pair.forall(vowels.contains)
